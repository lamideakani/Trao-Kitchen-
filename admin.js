 ...
 // Stock
      ...
      function loadCustomerData() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key.startsWith(userPrefix) || isValidJSON(item)) {
            try {
              ...
              if (user && user.username && user.email) {
                validUsers.push({
                  ...
                  email: user.email,
                  signupDate: user.signupDate || 'N/A' // Default to 'N/A' if no signup date is present
        ...
        // Populate the table with valid user data
